...
class Bert:
  def __init__(self, model_checkpoint="roberta-base"):
    ...
    self.tokenizer = RobertaTokenizerFast.from_pretrained(model_checkpoint, add_prefix_space = False)
  ...
  def mask_single_gender(self, gender_identifiers=[], input_text=""):
    ...
    input_text = (
        input_text[:single_match_start] + self.tokenizer.mask_token + input_text[single_match_end:]
    )
  ...
  def evaluate(self, eval_df):
    ...
    for prediction in tqdm(model_fn(KeyDataset(eval_df, "content"), top_k=TOP_K)):
      (woman_prob, man_prob) = self.compute_probs(prediction)
